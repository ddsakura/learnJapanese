...
```bash
npm run dev          # Start dev server (on macOS also auto-starts Ollama; otherwise start Ollama manually)
...
- `AppState` is `@MainActor`; tests that interact with it should also be `@MainActor`.
